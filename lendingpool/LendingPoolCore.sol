...
contract LendingPoolCore is VersionedInitializable {
    ...
    * @param _balanceIncrease the accrued interest on the borrowed amount
    * @param _liquidatorReceivesSToken true if the liquidator will receive sTokens, false otherwise
    **/
    function updateStateOnLiquidation(
        ...
        uint256 _balanceIncrease,
        bool _liquidatorReceivesSToken
    ) external onlyLendingPool {
    ...
    **/
    ...
    function transferToFeeCollectionAddress(
        ...
        if (_token != TrxAddressLib.trxAddress()) {
            ...
            ERC20(_token).transferFrom(_user, feeAddress, _amount);
        } else {
    ...
    **/
    function liquidateFee(
        ...
        if (_token != TrxAddressLib.trxAddress()) {
            ERC20(_token).transfer(feeAddress, _amount);
        } else {
            //solium-disable-next-line
            (bool result, ) = feeAddress.call.value(_amount).gas(50000)("");
            require(result, "Transfer of TRX failed");
    ...
    **/
    ...
    /**
    * @dev gets the underlying asset balance of a user based on the corresponding sToken balance.
    * @param _reserve the reserve address
    ...
    **/
    ...
    function getUserUnderlyingAssetBalance(address _reserve, address _user)
        ...
        returns (uint256)
    {
        SToken sToken = SToken(reserves[_reserve].sTokenAddress);
    ...
    **/
    ...
    /**
    * @dev gets the sToken contract address for the reserve
    * @param _reserve the reserve address
    * @return the address of the sToken contract
    **/

    function getReserveSTokenAddress(address _reserve) public view returns (address) {
        CoreLibrary.ReserveData storage reserve = reserves[_reserve];
        return reserve.sTokenAddress;
    }
    ...
    * @param _reserve the address of the reserve
    * @param _sTokenAddress the address of the overlying sToken contract
    ...
    **/
    function initReserve(
        ...
        reserves[_reserve].init(_sTokenAddress, _decimals, _interestRateStrategyAddress);
    ...
    **/
    function removeLastAddedReserve(address _reserveToRemove)
     external onlyLendingPoolConfigurator {
        ...
        reserves[lastReserve].sTokenAddress = address(0);
