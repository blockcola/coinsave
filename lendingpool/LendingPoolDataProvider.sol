...
contract LendingPoolDataProvider is VersionedInitializable {
    ...
    **/
    ...
    function getUserSaveInfo(address _user,address _reserve) public view returns(uint totalLiquidityBalanceTRX,uint totalBorrowBalanceTRX,uint sTokenLiquidityAmount,uint sTokenBorrowAmount){
        PriceOracle oracle = PriceOracle(addressesProvider.getPriceOracle());

        sTokenLiquidityAmount = SToken(core.getReserveSTokenAddress(_reserve)).balanceOf(_user);
        (, sTokenBorrowAmount, ) = core.getUserBorrowBalances(
            _reserve,
            _user
        );
   ...
   **/
    ...
    **/
        ...
        )
    {
        ...
        sTokenAddress = core.getReserveSTokenAddress(_reserve);
    ...
    {
        currentSTokenBalance = SToken(core.getReserveSTokenAddress(_reserve)).balanceOf(_user);
