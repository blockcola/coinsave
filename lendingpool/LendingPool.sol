...
import "../tokenization/SToken.sol";
...
contract LendingPool is ReentrancyGuard, VersionedInitializable {
    ...
    * @param _receiveSToken true if the liquidator wants to receive sTokens, false otherwise
    * @param _timestamp the timestamp of the action
    **/
    ...
    * @param _reserve the address of the reserve
    **/
    modifier onlyOverlyingSToken(address _reserve) {
        require(
            msg.sender == core.getReserveSTokenAddress(_reserve),
            "The caller of this function can only be the sToken contract of this reserve"
        );
        _;
    }
    ...
    **/
    ...
    /**
    * @dev deposits The underlying asset into the reserve. A corresponding amount of the overlying asset (sTokens)
    * is minted.
    * @param _reserve the address of the reserve
    * @param _amount the amount to be deposited
    ...
    **/
    ...
    {
        SToken sToken = SToken(core.getReserveSTokenAddress(_reserve));

        bool isFirstDeposit = sToken.balanceOf(msg.sender) == 0;

        core.updateStateOnDeposit(_reserve, msg.sender, _amount, isFirstDeposit);

        //minting SToken to user 1:1 with the specific exchange rate
        sToken.mintOnDeposit(msg.sender, _amount);
    ...
    * This function is executed by the overlying sToken contract in response to a redeem action.
    * @param _reserve the address of the reserve
    ...
    **/
    function redeemUnderlying(
        ...
        uint256 _amount,
        uint256 _sTokenBalanceAfterRedeem
    )
        external
        nonReentrant
        onlyOverlyingSToken(_reserve)
        onlyActiveReserve(_reserve)
        onlyAmountGreaterThanZero(_amount)
    {
        ...
        core.updateStateOnRedeem(_reserve, _user, _amount, _sTokenBalanceAfterRedeem == 0);
    ...
    * @param _receiveSToken true if the liquidators wants to receive the sTokens, false if
